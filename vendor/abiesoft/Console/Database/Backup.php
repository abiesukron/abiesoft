...
class Backup
{
    ...
    protected static function makeBackup($tabel, $j, $namabackupan)
    {
        ...
        $nama = $tabel;
        $namafolderBackup = "";
        for ($nf = 0; $nf < count(explode(" ", Config::envReader('WEB_TITLE'))); $nf++) {
            $namafolderBackup .= strtolower(str_replace(".", "", explode(" ", Config::envReader('WEB_TITLE'))[$nf]));
        }
        ...
        fwrite($file, $isi);
        $isi = "use AbieSoft\Application\Utilities\Generate;\n\n";
        fwrite($file, $isi);
        ...
        foreach ($cekkolom as $k) {
            if ($k->COLUMN_NAME !== "id" and $k->COLUMN_NAME !== "dibuat" and $k->COLUMN_NAME !== "diupdate") {
                if ($k->DATA_TYPE == "varchar") {
                    ...
                    $isi = "        $" . "" . "schema->teks(nama: '" . $namakolom . "'$panjang$default$unique);\n";
                    fwrite($file, $isi);
                }
                ...
                if ($k->DATA_TYPE == "int") {
                    ...
                    $isi = "        $" . "" . "schema->number(nama: '" . $namakolom . "'$default);\n";
                    fwrite($file, $isi);
                }

                if ($k->DATA_TYPE == "text") {
                    $namakolom = $k->COLUMN_NAME;
                    $isi = "        $" . "" . "schema->paragrap(nama: '" . $namakolom . "');\n";
                    fwrite($file, $isi);
                }

                if ($k->DATA_TYPE == "datetime") {
                    $namakolom = $k->COLUMN_NAME;
                    $isi = "        $" . "" . "schema->tanggal(nama: '" . $namakolom . "');\n";
                    fwrite($file, $isi);
                }
            }
        }
