...
if (password) {
    for (let i = 0; i < password.length; i++) {
        password[i].parentElement.children[1].addEventListener('click', () => {
            if (password[i].parentElement.children[1].classList.contains('la-eye')) {
                password[i].parentElement.children[1].classList.remove('la-eye');
                password[i].parentElement.children[1].classList.add('la-eye-slash');
                password[i].setAttribute('type', 'text');
            } else {
                password[i].parentElement.children[1].classList.remove('la-eye-slash');
                password[i].parentElement.children[1].classList.add('la-eye');
...
$(document).ready(function () {
    $(window).keydown(function (event) {
        if (event.keyCode == 8) {
            ...
            if (valueInput < 1 && event.target.dataset.type == "tag") {
                event.target.value = "";
