function msg(msg){
    const form = document.getElementById(msg[0]);
    const merror = document.getElementById('err_' + msg[0]);
    form.parentElement.classList.add("is-error");
    merror.innerHTML = "<span>"+msg[1]+"</span>";
    form.focus();
    if(form.type == "file"){
        form.setAttribute("onChange", "clearError('"+msg[0]+"')");
    }else if(form.tagName == "SELECT"){
        form.setAttribute("onChange", "clearError('"+msg[0]+"')");
    }else {
        form.setAttribute("onKeyDown", "clearError('"+msg[0]+"')");
    }
}

function clearError(x){
    const form = document.getElementById(x);
    const merror = document.getElementById('err_' + x);
    form.parentElement.classList.remove("is-error");
    merror.innerHTML = "";
}
